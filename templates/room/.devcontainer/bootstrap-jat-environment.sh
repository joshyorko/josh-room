...
if oras pull "$JAT_ENVIRONMENT_ARTIFACT_REFERENCE" --output "$stage" >/dev/null 2>&1 \
    && [[ -f $archive && -f $receipt ]] \
    && jq -e --arg jat "$JAT_GIT_SHA" --arg rcc "$JAT_ENVIRONMENT_ARTIFACT_RCC_VERSION" \
      ...
      '.success == true and .jat_git_sha == $jat and .rcc_version == $rcc and .platform == $platform and
       .manifest_digest == $manifest and .rcc_artifact_digest == $artifact and
       .rcc_specification_digest == $spec and .legacy_blueprint_key == $legacy and
    ...
    && jq -e --arg digest "$JAT_ENVIRONMENT_ARTIFACT_DIGEST" '.success == true and .verified == true and .artifact_digest == $digest' <<<"$acquired" >/dev/null \
