...
Windows uses a user Task Scheduler task. Definitions contain an absolute executable,
no inherited secrets, and no root requirement. Installation is idempotent; removal is
reversible. Unsupported platforms return the stable diagnostic
`scheduler-unsupported-platform`. Definitions are one-shot and guarded against
overlap by the native scheduler contract.
