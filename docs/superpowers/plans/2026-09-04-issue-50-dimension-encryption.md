...
- [ ] **Step 5: Publish the Draft PR as soon as the first implementation head is independently verified.** Push `patchraptor/issue-50-encryption-domains` and open a GitHub Draft PR against `main`; the PR must remain Draft. Do not merge, tag, release, direct-push `main`, or run live production migration.
...
- [ ] **Step 7: Run authorized synthetic verticals and final gates.** Exercise fresh MinIO initialization, cache deletion/re-enrollment, legacy shared-object migration with interrupt/resume and cutover conflict, cross-Dimension copy, and R2 fixture regression; then run the complete local, RCC, VSIX, parity, and secret-scan gates. Do not touch the user’s live bucket.
